...
- `tenant` — Postgres `zms_tenants` (identity/tenancy; supersedes `user-management`)
- `meet` — Postgres `zms_meetings`, Kafka, LiveKit; gRPC client → `user-management`
- `record` — Postgres `zms_recordings`, LiveKit egress → RustFS (S3-compatible)
...
**Wiring:** Kong gateway (external) · gRPC (`meet` → `user-management`) · Kafka +
CloudEvents (async, consumed by `notification`) · SSE + Valkey (real-time).
Integrations: LiveKit (WebRTC + egress), Firebase, Resend.
...
- Baseline: each service starts from a single `B1.0.0__baseline.sql` (Flyway `B`
  prefix — applied only on a clean DB). Incremental changes use `V<n>__<desc>.sql`
  (e.g. `V2__...`).
