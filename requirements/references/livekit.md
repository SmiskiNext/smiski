...
Tài liệu tham chiếu nội bộ về LiveKit phục vụ cho `meeting-management`
(video, gRPC, SSE) trong dự án smiski. Tổng hợp từ tài liệu chính thức
...
- Các field cập nhật được: `can_subscribe`, `can_publish`, `can_publish_data`,
  `can_publish_sources`, `hidden`, `can_update_metadata`, `can_subscribe_metrics`.
...
- **Revocation** (LiveKit Cloud): khi permission đổi hoặc participant bị xóa,
  token cũ tự bị revoke qua `revoke_token_ts` (Unix timestamp, phải nằm trong
  60 giây so với server time). `RemoveParticipant` API cũng có thể pass
...
LiveKit có embedded TURN server, relay media khi kết nối trực tiếp thất bại
(hữu ích với firewall doanh nghiệp chặn UDP hoặc TCP non-secure).
...
Output hỗ trợ: MP4 file, HLS segments, RTMP/SRT stream, thumbnails,
upload S3/Azure/GCP.
...
> filter out participant `kind = EGRESS`. Nếu không, `UpdateSubscriptions()`
> sẽ ghi đè subscription của recorder.
...
Lệnh này tạo room, thêm 4 virtual publishers và mở browser với URL đầy đủ params.
