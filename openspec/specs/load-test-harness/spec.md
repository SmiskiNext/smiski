...
Defines the WebRTC load-test and measurement harness that produces empirical
performance, reliability, and NAT-traversal evidence for the meeting stack. The
harness composes the development Docker stack without modifying it, provides
locally-signed gateway authentication, network-impairment profiles, a TURN
relay, meeting fixture seeding, k6-based HTTP load generation, LiveKit room
capacity simulation, a browser-based media quality client, per-container
resource metrics collection, and machine-readable evidence export — all running
from pinned container images with no host tool installation beyond a container
runtime.
