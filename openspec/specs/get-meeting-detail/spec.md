...
(non-removed) invitees. Each entry SHALL carry `accountId`, `email`,
`displayName`, `status` (the RSVP status), `invitedAt`, and `respondedAt`
(nullable when the invitee has not yet responded).
...
- **THEN** the `invitees` array contains one entry per active invitee, each with
  its `accountId`, `email`, `displayName`, `status`, `invitedAt`, and
  `respondedAt`
