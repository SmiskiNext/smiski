...
## Requirement: Staging overlay provides working development secrets

The staging overlay (`services/k8s/overlays/staging/`) SHALL include a
`secrets/` subdirectory containing Secret YAML files appropriate for
local/development use, with the same connection values previously hardcoded in
base manifests.

### Scenario: Staging overlay renders a complete set of secrets

- **WHEN** `kustomize build services/k8s/overlays/staging` is executed
- **THEN** the rendered output SHALL include Secret objects for all four
  services plus LiveKit and RustFS

### Scenario: Staging overlay kustomization references its secret files

- **WHEN** `services/k8s/overlays/staging/kustomization.yaml` is inspected
