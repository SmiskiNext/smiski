# nx-orchestration Specification

## Purpose

Defines how Nx orchestrates the monorepo build graph, enabling affected-scoped
task execution across Gradle services, pnpm packages, and repository tooling. Nx
discovers projects through the `@nx/gradle` plugin and pnpm workspace
integration, so every `nx affected` run builds, tests, or lints exactly the
projects touched by a change — and no more.
