...
string). When the body is absent or empty, the endpoint SHALL behave as if all
...
- **WHEN** a client sends `POST /api/1/meetings` with a valid `X-Tenant-ID`, an
  `X-Account-Id` header, and an empty JSON body `{}`
- **THEN** the response is `200 OK` containing meetings from that tenant ordered
  newest-first by creation time, limited to the default page size

#### Scenario: Missing account header is rejected

- **WHEN** a client sends `POST /api/1/meetings` without an `X-Account-Id`
  header
- **THEN** the request fails with a `400` Problem Details response and no list
  is returned

#### Scenario: Only the caller's tenant meetings are visible

- **WHEN** meetings exist across multiple tenants and a client lists with a
  specific `X-Tenant-ID`
- **THEN** the response contains only meetings whose tenant matches the request
  tenant, and never meetings from other tenants
...
`shortCode`, `title`, `description`, `issueKey`, `type`, `status`, `startTime`
(null for meetings without a scheduled start), `endTime` (nullable),
`createdAt`, and `settings`. The summary SHALL NOT include the tenant
identifier.
...
- **THEN** each returned item includes `id`, `hostId`, `shortCode`, `title`,
  `issueKey`, `type`, `status`, `startTime`, `endTime`, `createdAt`, and
  `settings`, and does not include the tenant identifier
