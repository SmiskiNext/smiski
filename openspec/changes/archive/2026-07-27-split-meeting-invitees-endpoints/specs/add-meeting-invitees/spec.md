...
more new invitees to a meeting. The acting account SHALL be resolved from the
configured account header, the tenant SHALL be resolved from the tenant context,
and only the meeting host SHALL be authorized to add invitees. The request body
SHALL use the shape
...
- **WHEN** the host sends a valid `POST /api/1/meetings/{id}/invitees` request
  with the account header and one or more invitees that are not yet active
...
#### Scenario: Non-host addition is rejected

- **WHEN** an account that is not the meeting host sends an invitee-creation
  request
- **THEN** the system returns an RFC 9457 Problem Details response with the
  authorization error code and HTTP `403`, creates no invitee, and publishes no
  event

#### Scenario: Missing account identity is rejected

- **WHEN** the request does not contain the configured account header
- **THEN** the system returns `400` Problem Details and creates no invitee
