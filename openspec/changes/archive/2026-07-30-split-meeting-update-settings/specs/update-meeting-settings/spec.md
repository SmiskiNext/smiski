...
entire meeting settings block. The acting account SHALL be resolved from the
configured account header, the tenant SHALL be resolved from the tenant context,
and only the meeting host SHALL be authorized to change settings. The request
SHALL carry the full settings representation (`admissionPolicy`,
`maxParticipants`, `allowMicrophone`, `allowVideo`, `allowScreenShare`,
`chatEnabled`). A successful replacement SHALL return `200 OK` with the updated
settings snapshot.
...
- **WHEN** an account that is not the meeting host sends a settings request
...
- **WHEN** the settings request does not contain the configured account header
- **THEN** the system returns `400` Problem Details and does not change the
  settings

#### Scenario: Unknown meeting is rejected

- **WHEN** the settings request references an ID that does not exist in the
  tenant
