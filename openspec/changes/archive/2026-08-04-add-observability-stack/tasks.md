...
- [x] 6.7 _Default startup is unaffected_ — run the default startup command and
      ...
      Both lists `diff` clean:

      ```text
                              envoy gateway kafka livekit-redis livekit-server meet meet-postgres
                              notification notification-postgres tenant tenant-postgres valkey
                              ```

                              **Corrected in design Context and the proposal, which said "eleven".** The
                              default set is twelve, before and after — `config --services` returns 12 on
                              both, and 20 with the profile. The count in Context is off by one; the
                              change's actual guarantee, that the set is unchanged, holds.
