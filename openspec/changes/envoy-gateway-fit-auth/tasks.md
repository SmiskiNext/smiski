...
- [ ] 12.1 Start docker compose stack with all services
- [ ] 12.2 Verify Envoy listening on port 8080
- [ ] 12.3 Verify gateway gRPC server listening on port 9001
- [ ] 12.4 Verify Valkey accessible on port 6379
- [ ] 12.5 Test request with valid FIT to /api/1/meetings (expect auth success)
- [ ] 12.6 Verify gateway logs show FIT parsed, cache miss, Jira called
- [ ] 12.7 Verify meet service receives X-Tenant-ID, X-Account-Id,
      X-Project-Permissions headers
- [ ] 12.8 Test same request again (expect cache hit, no Jira call)
- [ ] 12.9 Test request with invalid FIT signature (expect 401 from Envoy)
- [ ] 12.10 Test request with expired FIT (expect 401 from Envoy)
- [ ] 12.11 Test request without X-Issue-Id header (expect empty permissions)
- [ ] 12.12 Test request to /api/1/tenants routes to tenant:8081
- [ ] 12.13 Test request to /api/1/notifications routes to notification:8083
- [ ] 12.14 Stop Valkey and verify cache bypass works
- [ ] 12.15 Stop Jira API mock and verify stale cache fallback works ← (verify:
      full request flow works end-to-end)

## 13. Forge Deployment

- [ ] 13.1 Run `cd app && pnpm build`
- [ ] 13.2 Run `forge deploy --non-interactive -e development`
- [ ] 13.3 Verify deployment succeeds without errors
- [ ] 13.4 Run `forge install --upgrade --site <test-site> --product jira`
- [ ] 13.5 Verify upgrade completes successfully
- [ ] 13.6 Open Forge app in Jira issue panel
- [ ] 13.7 Open browser DevTools network tab
- [ ] 13.8 Trigger API call from UI (e.g., create meeting)
- [ ] 13.9 Verify request includes X-Issue-Id and X-Project-Key headers
- [ ] 13.10 Verify request includes x-forge-oauth-system header
- [ ] 13.11 Check gateway logs for permission check
- [ ] 13.12 Verify backend service receives injected headers ← (verify: Forge
      app works end-to-end with gateway)

## 14. Manual Testing

- [ ] 14.1 Test user with view-meeting permission can read meetings
- [ ] 14.2 Test user with edit-meeting permission can create/update meetings
- [ ] 14.3 Test user without permissions receives 403 from backend
- [ ] 14.4 Test permission check for different issues in same project
- [ ] 14.5 Test permission check for different projects
- [ ] 14.6 Test cache expiration after 15 minutes triggers fresh Jira call
- [ ] 14.7 Test concurrent requests from multiple users
- [ ] 14.8 Test gateway handles high request rate (50+ req/sec)
- [ ] 14.9 Monitor Valkey for cached permission entries
- [ ] 14.10 Check Envoy admin interface at :9901 for metrics ← (verify: all
      permission scenarios work correctly)

## 15. Documentation

- [ ] 15.1 Create `services/gateway/README.md` with service overview
- [ ] 15.2 Document environment variables in README
- [ ] 15.3 Document Go dependencies and versions
- [ ] 15.4 Document gRPC server endpoints and health check
- [ ] 15.5 Add deployment instructions to root AGENTS.md
- [ ] 15.6 Document Envoy configuration structure
- [ ] 15.7 Document cache key format and TTL
- [ ] 15.8 Document Jira API integration and error handling
- [ ] 15.9 Add troubleshooting section for common issues
- [ ] 15.10 Document rollback procedure ← (verify: documentation complete and
      accurate)
