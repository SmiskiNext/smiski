...
export function meetingFromBackend(payload: unknown): Meeting {
  ...
  const projectKey = issueLink.projectKey ?? snapshot.projectKey ?? issueKey.split('-')[0] ?? '';
  const status = normalizeMeetingStatus(snapshot.status);
  const hostId = snapshot.hostId ?? snapshot.creatorId ?? '';
  const hostName =
    snapshot.hostName ?? snapshot.organizerDisplayName ?? snapshot.creatorName ?? 'Unknown host';
  ...
  return {
    ...
    status,
    participantCount: snapshot.participantCount ?? snapshot.inviteeCount ?? 1,
  };
}

export function meetingsFromBackend(payload: unknown): Meeting[] {
  return extractArray<BackendMeetingSnapshot>(payload, ['meetings', 'items', 'content', 'data']).map(
    meetingFromBackend,
  );
}
...
export function participantsFromBackend(payload: unknown): Participant[] {
  return extractArray<BackendParticipantSnapshot>(payload, [
    ...
    displayName: participant.displayName ?? participant.accountId ?? 'Unknown participant',
...
export function recordingFromBackend(payload: unknown): Recording | null {
  ...
  const snapshot = isEnvelope(payload) && payload.recording ? payload.recording : payload;
...
export function roomTokenFromBackend(payload: unknown): { token: string; url: string } {
  const source = liveKitFromBackend(payload);
  const token = source.token;
  const url = source.url ?? source.livekitUrl ?? apiConfig.liveKitUrl;
  if (!token || !url) throw new Error('Backend did not return LiveKit token and URL.');
  return { token, url };
}
...
export function permissionsFromBackend(payload: unknown): {
  ...
  if (!isEnvelope(payload)) return { hasViewMeeting: false, hasEditMeeting: false };
  return {
    hasViewMeeting: Boolean(
      payload.hasViewMeeting ?? payload.canViewMeeting ?? payload.viewMeeting,
    ),
    hasEditMeeting: Boolean(
      payload.hasEditMeeting ?? payload.canEditMeeting ?? payload.editMeeting,
    ),
  };
}
...
export function scheduleMeetingRequest(
  ...
  return {
    ...
    issueLink: issueLinkFromInput(input, context),
    settings: DEFAULT_MEETING_SETTINGS,
    timeRange: timeRangeFromStart(input.startTime, input.endTime, input.durationMinutes),
...
export function updateMeetingRequest(
  ...
  if (!startTime) throw new Error('A start time is required to update a backend meeting.');
  ...
  return {
    ...
    settings: DEFAULT_MEETING_SETTINGS,
    zoneId: input.zoneId ?? getLocalTimeZone(),
    timeRange: timeRangeFromStart(startTime, input.endTime, input.durationMinutes),
  };
}
...
function issueLinkFromInput(
  ...
  const projectKey = input.projectKey ?? context.projectKey ?? input.issueKey.split('-')[0];
  return {
...
function issueLinkFromMeeting(
  ...
  if (!issueKey) throw new Error('An issue key is required to update a backend meeting.');
  const projectKey =
    input.projectKey ?? context.projectKey ?? meeting?.projectKey ?? issueKey.split('-')[0];
  return {
    issueId: input.issueId ?? context.issueId ?? meeting?.issueId ?? `issue-${issueKey}`,
    issueKey,
    projectKey,
  };
}
...
function inviteesFromAccountIds(
  ...
  const members = new Map(
    (context.projectMembers ?? []).map((member) => [member.accountId, member]),
  );
...
function emailFor(member: ProjectMember | undefined): string {
  ...
  const safe = accountId.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '');
...
function normalizeMeetingStatus(value: string | undefined): MeetingStatus {
  if (value === 'RUNNING' || value === 'COMPLETED' || value === 'CANCELED') return value;
...
function normalizeRecordingStatus(value: string | undefined): RecordingStatus {
  if (value === 'RECORDING' || value === 'COMPLETED' || value === 'FAILED') return value;
  return 'PENDING';
}
