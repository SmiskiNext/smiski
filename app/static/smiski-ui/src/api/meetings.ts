...
export async function getIssueMeetings(issueKey: string): Promise<Meeting[]> {
  const payload = await apiRequest<unknown>(meetingEndpoints.list, { query: { issueKey } });
  return meetingsFromBackend(payload);
}

/** List/search meetings across a project (dashboard). */
export async function getProjectMeetings(filters: MeetingListFilters): Promise<Meeting[]> {
  const payload = await apiRequest<unknown>(meetingEndpoints.list, { query: { ...filters } });
  return meetingsFromBackend(payload);
}
...
export async function updateMeeting(
  ...
  const payload = await apiRequest<unknown>(meetingEndpoints.update(meetingId), {
    method: 'PUT',
...
export async function cancelMeeting(meetingId: string): Promise<void> {
  await apiRequest<unknown>(meetingEndpoints.cancel(meetingId), { method: 'POST' });
}

/** Transition a scheduled meeting to RUNNING (start). */
export async function startMeeting(meetingId: string): Promise<Meeting> {
  const payload = await apiRequest<unknown>(meetingEndpoints.start(meetingId), { method: 'POST' });
  return payload == null ? getMeeting(meetingId) : meetingFromBackend(payload);
}

/** Transition a running meeting to COMPLETED. */
export async function endMeeting(meetingId: string): Promise<Meeting> {
  const payload = await apiRequest<unknown>(meetingEndpoints.end(meetingId), { method: 'POST' });
  return payload == null ? getMeeting(meetingId) : meetingFromBackend(payload);
}
...
export async function getRoomToken(meetingId: string): Promise<{ token: string; url: string }> {
  if (shouldUseBackendApi()) {
    const payload = await apiRequest<unknown>(meetingEndpoints.roomToken(meetingId), {
      method: 'POST',
    });
