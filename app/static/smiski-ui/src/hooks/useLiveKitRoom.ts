...
export type LiveKitConnectionState = 'idle' | 'connecting' | 'connected' | 'disconnected' | 'error';
...
function toParticipant(participant: LKParticipant, isLocal: boolean): LiveMeetingParticipant {
  ...
  return {
    ...
    videoTrack: (videoPub?.track as LocalVideoTrack | RemoteVideoTrack | undefined) ?? null,
    audioTrack: (audioPub?.track as LocalAudioTrack | RemoteAudioTrack | undefined) ?? null,
  };
}

export function useLiveKitRoom({ token, url, enabled }: UseLiveKitRoomOptions): UseLiveKitRoomResult {
  ...
  const snapshot = useCallback(() => {
    ...
    const remotes = Array.from(room.remoteParticipants.values()).map((p) => toParticipant(p, false));
    ...
    (async () => {
      try {
        ...
        try {
          ...
          console.warn('[useLiveKitRoom] camera/mic unavailable', mediaError);
        }
        if (cancelled) return;
        snapshot();
  ...
  return {
    ...
    isScreenSharing: roomRef.current?.localParticipant.isScreenShareEnabled ?? false,
    toggleMic,
    toggleCamera,
    toggleScreenShare,
    leave,
