...
export function useMeetingParticipants(meetingId?: string, projectKey?: string) {
  const currentUser = useCurrentUser();
  const projectMembers = useProjectMembers(projectKey);
  const query = useQuery({
    queryKey: meetingId ? queryKeys.participants(meetingId) : ['participants', 'none'],
  ...
  return {
    participants,
    loading: query.isLoading || (Boolean(projectKey) && projectMembers.loading),
