    ...
    >
      {isLoading ? (
        ...
        <div className="space-y-5">
          ...
          {meeting.description && (
            <p className="text-sm leading-6 text-[var(--text-muted)]">{meeting.description}</p>
          )}
          <dl className="grid gap-3 rounded-2xl bg-[var(--surface-soft)] p-4 text-sm sm:grid-cols-2">
            <div>
              <dt className="text-xs text-[var(--text-faint)]">Host</dt>
              <dd className="mt-1 font-semibold text-[var(--text)]">{meeting.hostName}</dd>
            </div>
            <div>
              <dt className="text-xs text-[var(--text-faint)]">Starts</dt>
              <dd className="mt-1 inline-flex items-center gap-1.5 font-semibold text-[var(--text)]">
                <Icon name="clock" size={15} />
                {meeting.scheduledAt || meeting.startedAt
                  ? new Date(meeting.scheduledAt ?? meeting.startedAt ?? '').toLocaleString()
                  : 'Not set'}
              </dd>
            </div>
          </dl>
          <div>
            ...
            {participants.length ? (
              <ParticipantAvatarGroup participants={participants} />
            ) : (
              <p className="text-sm text-[var(--text-muted)]">No participants recorded.</p>
            )}
          </div>
        </div>
      )}
