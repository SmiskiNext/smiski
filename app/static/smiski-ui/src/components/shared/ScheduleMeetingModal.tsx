...
type FieldErrors = Partial<
    Record<'issueKey' | 'title' | 'date' | 'time' | 'form', string>
>;
...
export function ScheduleMeetingModal({
    ...
    const { members, loading: membersLoading } =
        useProjectMembers(effectiveProjectKey);
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: form/date/time state intentionally resets only on open, not when initialDate/initialTime change
    useEffect(() => {
        if (isOpen) {
            setDate(initialDate);
            setTime(initialTime);
            if (isEdit || issueKey) titleRef.current?.focus();
        }
    }, [isOpen, meeting?.id]);
    ...
    const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
        ...
        const resolvedIssueKey = (issueKey ?? pickedIssueKey)
            .trim()
            .toUpperCase();
