...
export function IssuePicker({
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      >
        <Icon name="search" size={14} className="shrink-0 text-[var(--text-faint)]" />
        <input
      ...
      {value && !isOpen && selectedSummary && (
        <p className="mt-1 truncate text-xs text-[var(--text-faint)]">{selectedSummary}</p>
      )}
      ...
      {isOpen &&
        ...
        createPortal(
          ...
          >
          {loading ? (
            <p className="px-2.5 py-3 text-xs text-[var(--text-faint)]">Loading issues…</p>
          ) : error ? (
            <p className="px-2.5 py-3 text-xs text-red-600 dark:text-red-300">{error.message}</p>
          ) : issues.length === 0 ? (
            ...
            issues.map((issue) => {
              ...
              return (
                <button
                  ...
                  onClick={() => select(issue.key, issue.summary)}
                >
                  <span className="shrink-0 text-xs font-semibold text-brand-700 dark:text-brand-300">
                    {issue.key}
                  </span>
                  <span className="truncate text-xs text-[var(--text-muted)]">{issue.summary}</span>
                  {isSelected && (
                    <Icon name="check" size={14} className="ml-auto shrink-0 text-brand-600" />
                  )}
                </button>
              );
            })
