...
function loadMeetings(): Meeting[] {
  try {
    ...
    return Array.isArray(parsed) && parsed.length > 0 ? parsed : seedMeetings();
  } catch {
    return seedMeetings();
  }
}
 ...
 */
export function migrateLegacyMockCurrentUser(currentUser: ProjectMember): boolean {
  if (!currentUser.accountId || currentUser.accountId === CURRENT_USER.accountId) return false;
  ...
  meetings = meetings.map((meeting) => {
    ...
    return {
      ...
      creatorName: ownsCreator ? currentUser.displayName : meeting.creatorName,
  ...
  if (changed) {
    ...
    migrateMockParticipantIdentity(migratedMeetingIds, CURRENT_USER.accountId, currentUser);
  }
  return changed;
}

export async function listIssueMeetings(issueKey: string): Promise<Meeting[]> {
  return delay(meetings.filter((m) => m.issueKey === issueKey).map(cloneMeeting));
}

export async function listProjectMeetings(filters: MeetingListFilters): Promise<Meeting[]> {
  ...
  const result = meetings.filter((m) => {
    ...
    if (filters.createdByAccountId && m.creatorId !== filters.createdByAccountId) return false;
...
export async function findRunningMeetingHostedByUser(
  ...
  const hit = meetings.find(
    (m) => m.status === 'RUNNING' && m.hostId === accountId && m.issueKey !== excludingIssueKey,
  );
...
export async function createInstantMeeting(
  ...
  persistMeetings(meetings);
  setMockParticipants(meeting.id, actor, input.participantAccountIds, identity.projectMembers);
  return delay(cloneMeeting(meeting));
}
...
export async function cancelMeeting(meetingId: string): Promise<void> {
  meetings = meetings.map((m) => (m.id === meetingId ? { ...m, status: 'CANCELED' } : m));
  persistMeetings(meetings);
...
export async function startMeeting(meetingId: string): Promise<Meeting> {
  ...
  meetings = meetings.map((m) => {
    if (m.id !== meetingId) return m;
    updated = { ...m, status: 'RUNNING', startedAt: new Date().toISOString() };
    return updated;
  });
  persistMeetings(meetings);
  if (!updated) throw new Error(`Mock meeting not found: ${meetingId}`);
...
export async function endMeeting(meetingId: string): Promise<Meeting> {
  ...
  meetings = meetings.map((meeting) => {
    ...
    updated = { ...meeting, status: 'COMPLETED', endedAt: new Date().toISOString() };
    return updated;
  });
  persistMeetings(meetings);
  if (!updated) throw new Error(`Mock meeting not found: ${meetingId}`);
...
export async function listMeetingParticipants(meetingId: string): Promise<Participant[]> {
