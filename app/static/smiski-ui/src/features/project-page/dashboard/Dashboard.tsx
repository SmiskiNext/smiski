...
import { SearchAndFilterBar, type MeetingFilterValue } from './SearchAndFilterBar';
  ...
  const handleAction = (action: MeetingAction, meeting: Meeting) => {
    switch (action) {
      ...
      case 'CANCEL':
        ...
        break;
      case 'START':
        startMeeting.mutate(meeting.id, { onSuccess: (started) => onOpenRoom(started.id) });
        break;
      case 'JOIN':
  ...
  return (
    <div className="w-full bg-[var(--surface)]">
      ...
      <div className="border-b bg-[var(--surface)] px-4 py-2.5 sm:px-6">
        <SearchAndFilterBar projectKey={projectKey} value={filters} onChange={setFilters} />
      </div>
      {feedback && (
      ...
      <MeetingListTable
        ...
        isLoading={loading}
        error={error}
        onSelect={(meeting) => setSelectedMeeting({ id: meeting.id, focus: 'details' })}
