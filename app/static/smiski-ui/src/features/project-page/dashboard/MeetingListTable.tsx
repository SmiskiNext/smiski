...
const columns: Array<{
  ...
  { key: 'issueKey', label: 'Issue', sortable: true, className: 'hidden w-[18%] sm:table-cell' },
  ...
  { key: 'participants', label: 'People', className: 'hidden w-16 2xl:table-cell' },
  ...
  return (
    <section>
      <div className="flex h-10 items-center justify-between border-b px-4 sm:px-6">
        <h2 className="text-xs font-semibold text-[var(--text)]">All meetings</h2>
      ...
      {isLoading ? (
        ...
        <div className="p-6">
          <EmptyState header="No meetings found" description="Try adjusting the active filters." />
        </div>
      ) : (
        <div>
            ...
            <tbody>
              {pagedMeetings.map((meeting) => (
                <tr
                  ...
                  onKeyDown={(event) => {
                    if (event.key === 'Enter') onSelect(meeting);
                  }}
                >
                  <td className="border-b px-3 py-2.5 pl-6">
                    ...
                    <p className="mt-0.5 max-w-sm truncate text-[11px] text-[var(--text-faint)]">
                      {meeting.description ?? `Created by ${meeting.creatorName}`}
                    </p>
                  </td>
                  ...
                  <td className="border-b px-3 py-2">
                    <ActionsCell meeting={meeting} permissions={permissions} onAction={onAction} />
                  </td>
                </tr>
              ))}
            </tbody>
            ...
            <div className="flex items-center justify-between border-t px-4 py-2.5 sm:px-6">
              ...
              <div className="flex items-center gap-1.5">
                <Button
                  ...
                  leadingIcon={<Icon name="chevronDown" size={14} className="rotate-90" />}
                >
                  Previous
                </Button>
                <Button
