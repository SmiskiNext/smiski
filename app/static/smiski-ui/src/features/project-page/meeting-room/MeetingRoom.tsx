...
export function MeetingRoom({ meetingId, onLeave }: MeetingRoomProps) {
  ...
  const { token, url, error: roomTokenError } = useRoomToken(meetingId, isLiveKitEnabled);
  ...
  const roomParticipants: (Participant | (typeof liveKit.participants)[number])[] =
    ...
    <div className="w-full px-4 py-4 sm:px-6">
      <div className="mb-3 flex items-center justify-between">
        <Button
          ...
          leadingIcon={<Icon name="chevronDown" size={15} className="rotate-90" />}
          onClick={handleLeave}
        >
          Back to meetings
        </Button>
      ...
      <div className="flex flex-col gap-4 xl:flex-row">
        <div className="min-w-0 flex-1">
          <MeetingRoomShell
            ...
            onTogglePeoplePanel={() => setPeoplePanelOpen((value) => !value)}
            onLeave={handleLeave}
            isMicOn={isLiveKitEnabled ? liveKit.isMicOn : undefined}
            isCameraOn={isLiveKitEnabled ? liveKit.isCameraOn : undefined}
            isScreenSharing={isLiveKitEnabled ? liveKit.isScreenSharing : undefined}
            onToggleMic={isLiveKitEnabled ? liveKit.toggleMic : undefined}
            onToggleCamera={isLiveKitEnabled ? liveKit.toggleCamera : undefined}
            onToggleScreenShare={isLiveKitEnabled ? liveKit.toggleScreenShare : undefined}
            connectionState={isLiveKitEnabled ? liveKit.connectionState : undefined}
          />
        </div>
        {isPeoplePanelOpen && (
          <ParticipantListPlaceholder
            participants={participants.length ? participants : roomParticipants}
