...
function isLiveParticipant(participant: GridParticipant): participant is LiveMeetingParticipant {
...
function OverflowTile({ count }: { count: number }) {
  return (
    <div className="flex h-full w-full flex-col items-center justify-center gap-1 rounded-2xl border border-white/8 bg-slate-800 text-white">
      ...
      <span className="text-xs text-slate-400">{count === 1 ? 'more person' : 'more people'}</span>
    </div>
  );
}
...
export function ParticipantVideoGrid({
  ...
  const self = participants.find((participant) => participant.accountId === selfAccountId);
  const others = participants.filter((participant) => participant.accountId !== selfAccountId);

  const otherSlots = MAX_VISIBLE_TILES - (self ? 1 : 0);
  const overflowCount = Math.max(0, others.length - otherSlots);
  const visibleOthers = overflowCount > 0 ? others.slice(0, otherSlots - 1) : others;

  const cells: GridCell[] = [
    ...visibleOthers.map((participant): GridCell => ({ kind: 'participant', participant })),
    ...(overflowCount > 0 ? [{ kind: 'overflow', count: overflowCount } as const] : []),
    ...
    >
      {rows.map((row, rowIndex) => {
        const tileSize = containerWidth
          ? computeTileSize(containerWidth, containerHeight, row.length, rows.length)
          : null;
        return (
          <div key={rowIndex} className="flex flex-1 items-center justify-center gap-3">
            {row.map((cell) => {
              const style = tileSize
                ? { width: tileSize.width, height: tileSize.height }
                : { width: '100%', aspectRatio: TILE_ASPECT };
              return cell.kind === 'overflow' ? (
                <div key="overflow" style={style} className="shrink-0">
                  <OverflowTile count={cell.count} />
                </div>
              ) : (
                <div key={cell.participant.accountId} style={style} className="shrink-0">
                  <ParticipantVideoTile
                    participant={cell.participant}
                    isSelf={cell.participant.accountId === selfAccountId}
                    isMicOn={
                      isLiveParticipant(cell.participant)
                        ? cell.participant.isMicOn
                        : cell.participant.accountId === selfAccountId
                          ? isSelfMicOn
                          : undefined
                    }
                    isCameraOn={
                      isLiveParticipant(cell.participant) ? cell.participant.isCameraOn : undefined
                    }
                    videoTrack={
                      isLiveParticipant(cell.participant) ? cell.participant.videoTrack : undefined
                    }
                    audioTrack={
                      isLiveParticipant(cell.participant) ? cell.participant.audioTrack : undefined
                    }
                  />
                </div>
              );
