...
import type { LiveKitConnectionState, LiveMeetingParticipant } from '../../../hooks/useLiveKitRoom';
...
function ControlButton({
  ...
  label: string;
  icon: 'mic' | 'micOff' | 'camera' | 'cameraOff' | 'screen' | 'people' | 'phoneOff';
  active?: boolean;
  danger?: boolean;
  onClick?: () => void;
  badge?: number;
...
export function MeetingRoomShell({
  ...
  const handleToggleMic = onToggleMic ?? (() => setLocalMicOn((value) => !value));
  const handleToggleCamera = onToggleCamera ?? (() => setLocalCameraOn((value) => !value));
  ...
  return (
    <section className="overflow-hidden rounded-3xl border border-slate-700 bg-slate-950 shadow-panel">
          ...
          {connectionState && (
            <span
              className={cn(
                ...
                connectionState === 'connecting' && 'bg-amber-500/20 text-amber-300',
                connectionState === 'error' && 'bg-red-500/20 text-red-300',
                connectionState === 'connected' && 'bg-emerald-500/20 text-emerald-300',
      ...
      <footer className="flex items-center justify-center gap-3 border-t border-white/8 bg-slate-950 px-3 py-4 sm:gap-5">
        ...
        />
        <ControlButton label="Leave" icon="phoneOff" danger onClick={onLeave} />
      </footer>
    </section>
  );
}
