...
export function IssuePanelRoot({ issue, onDevNavigateToProjectPage }: IssuePanelRootProps) {
  return (
    <main className="mx-auto flex w-full max-w-md flex-col bg-[var(--app-bg)]">
      <header className="flex items-center justify-between border-b bg-[var(--surface)] px-3 py-3 sm:px-4">
        <h1 className="text-lg font-semibold tracking-tight text-[var(--text)]">Meetings</h1>
      ...
      <div className="flex flex-col px-3 py-4 sm:px-4">
        <IssueMeetingsPanel issue={issue} onDevNavigateToProjectPage={onDevNavigateToProjectPage} />
      </div>
    </main>
  );
}
