...
function meeting(
    ...
    title: string,
    timing: Partial<
        Pick<Meeting, 'scheduledAt' | 'startedAt' | 'endedAt'>
    > = {},
): Meeting {
    return {
        id,
...
describe('filterAndSortIssueMeetings', () => {
    it('filters by status', () => {
        const meetings = [
            meeting('1', 'RUNNING', 'A'),
            meeting('2', 'SCHEDULED', 'B'),
        ];
        expect(
            filterAndSortIssueMeetings(meetings, { status: 'SCHEDULED' }).map(
                (m) => m.id,
            ),
        ).toEqual(['2']);
    });
    ...
    it('filters by title, case-insensitively', () => {
        const meetings = [
            meeting('1', 'RUNNING', 'Sprint sync'),
            meeting('2', 'RUNNING', 'Retro'),
        ];
        expect(
            filterAndSortIssueMeetings(meetings, { search: 'sprint' }).map(
                (m) => m.id,
            ),
        ).toEqual(['1']);
    });
    ...
    it('sorts RUNNING before SCHEDULED before COMPLETED/CANCELED', () => {
        ...
        expect(
            filterAndSortIssueMeetings(meetings, {}).map((m) => m.id),
        ).toEqual(['running', 'scheduled', 'completed', 'canceled']);
    });

    it('orders SCHEDULED meetings soonest-first', () => {
        const meetings = [
            meeting('later', 'SCHEDULED', 'Later', {
                scheduledAt: '2026-01-02T00:00:00.000Z',
            }),
            meeting('sooner', 'SCHEDULED', 'Sooner', {
    ...
    it('orders COMPLETED meetings most-recent-first', () => {
        const meetings = [
            meeting('older', 'COMPLETED', 'Older', {
                endedAt: '2026-01-01T00:00:00.000Z',
            }),
            meeting('newer', 'COMPLETED', 'Newer', {
