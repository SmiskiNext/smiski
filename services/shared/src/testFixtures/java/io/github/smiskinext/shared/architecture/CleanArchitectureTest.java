package io.github.smiskinext.shared.architecture;
...
public abstract class CleanArchitectureTest {
    ...
    private static final String SHARED_INFRASTRUCTURE =
            "io.github.smiskinext.shared.infrastructure..";

    /**
     * Predicate that excludes Spring AOT-generated and CGLib proxy classes.
     *
     * <p>Spring generates classes like {@code Foo$$SpringCGLIB$$0} (CGLib proxies) and {@code
     * Foo__TestContext001_BeanDefinitions} (AOT context registrations). These are bytecode artifacts
     * and must not be treated as intentional architectural decisions.
     */
    ...
    /**
     * Presentation drives the application through use-case interfaces and must not reach into
     * infrastructure adapters (LiveKit, Kafka, JPA repositories) directly. Spring AOT-generated
     * classes are excluded from this check.
     */
    @ArchTest
    static final ArchRule presentation_must_not_depend_on_infrastructure = noClasses()
            ...
            .because("Presentation must drive the application through use-case interfaces,"
                    + " never depend on infrastructure adapters directly");

    /**
     * Use cases live in application.usecase and must be named with the UseCase suffix. Only
     * top-level types are checked so that nested request/response types are not forced to carry
     * the suffix. Spring AOT/CGLib generated proxy classes are excluded.
     */
    @ArchTest
    static final ArchRule use_cases_must_have_UseCase_suffix = classes()
            ...
            .allowEmptyShould(true)
            .because("All top-level types in application.usecase must be named *UseCase");

    /**
     * Application services implement use-case interfaces and must be named with the
     * ApplicationService suffix.
     */
    @ArchTest
    static final ArchRule application_services_must_have_ApplicationService_suffix = classes()
            .that()
            ...
            .allowEmptyShould(true)
            .because("Implementations of use-case interfaces in application.service are named"
                    + " *ApplicationService by convention");
    ...
    /**
     * @Entity belongs only in infrastructure.persistence — domain models must be
