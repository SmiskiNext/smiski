 ...
 *
 * <p>It feeds the value Hibernate uses to populate {@code @TenantId} columns on insert and to append
 * {@code WHERE tenant_id = ?} to queries. The resolution always yields a non-null value so Spring
 * Boot AOT processing and startup (which run without an HTTP request) do not fail.
 *
