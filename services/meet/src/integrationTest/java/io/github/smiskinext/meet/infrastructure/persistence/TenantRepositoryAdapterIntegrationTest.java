...
class TenantRepositoryAdapterIntegrationTest {
    ...
    void upsertInsertsNewTenantWithActiveStatus() {
        ...
        TenantRecord record =
                new TenantRecord("cloud-new", "cloud-new", TenantStatus.ACTIVE, now, null, null);
    ...
    void upsertUpdatesExistingTenantToUninstalled() {
        ...
        tenantRepositoryAdapter.upsert(
                new TenantRecord("cloud-up", "cloud-up", TenantStatus.ACTIVE, now, null, null));

        tenantRepositoryAdapter.upsert(new TenantRecord(
                "cloud-up", "cloud-up", TenantStatus.UNINSTALLED, now, now, purgeAfter));
    ...
    void upsertReactivatesUninstalledTenant() {
        ...
        Instant purgeAfter = now.plus(30, ChronoUnit.DAYS);
        tenantRepositoryAdapter.upsert(new TenantRecord(
                "cloud-re", "cloud-re", TenantStatus.UNINSTALLED, now, now, purgeAfter));

        tenantRepositoryAdapter.upsert(
                new TenantRecord("cloud-re", "cloud-re", TenantStatus.ACTIVE, now, null, null));
