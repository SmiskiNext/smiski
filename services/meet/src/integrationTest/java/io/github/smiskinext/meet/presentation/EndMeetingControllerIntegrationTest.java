...
class EndMeetingControllerIntegrationTest {
    ...
    @Test
    void hostEndCompletesRunningMeetingAndClosesParticipation() throws Exception {
        UUID meetingId = insertMeeting("RUNNING");
        insertActiveParticipationLog(meetingId, "participant-1");
        ...
        assertThat(jdbcTemplate.queryForObject(
                ...
                .isEqualTo("COMPLETED");
        assertThat(jdbcTemplate.queryForObject(
                        "SELECT left_at IS NOT NULL FROM participation_logs WHERE meeting_id = ?",
                        Boolean.class,
                        meetingId))
                .isTrue();
    ...
    @Test
    void nonHostEndIsForbiddenAndMissingAccountIsBadRequest() throws Exception {
        UUID meetingId = insertMeeting("RUNNING");
    ...
    @Test
    void scheduledMeetingEndIsConflict() throws Exception {
        UUID meetingId = insertMeeting("SCHEDULED");

        mockMvc.perform(post("/api/1/meetings/{id}:end", meetingId)
                        .header("X-Account-Id", HOST_ID)
                        .header("X-Tenant-ID", TENANT_ID))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.code").value("MEETING_NOT_RUNNING"));
    ...
    private UUID insertMeeting(String status) {
        ...
        jdbcTemplate.update(
                ...
                INSERT INTO meetings (
                    ...
                    'PROJ', 'Title', 'Description', 'UTC', 'INSTANT', ?, ?::jsonb, ?, ?)
                """,
                TENANT_ID,
                id,
                HOST_ID,
    ...
    private void insertActiveParticipationLog(UUID meetingId, String accountId) {
        jdbcTemplate.update(
                """
                INSERT INTO participation_logs (
                    tenant_id, id, meeting_id, account_id, role, livekit_identity, joined_at
                ) VALUES (?, ?, ?, ?, 'PARTICIPANT', ?, ?)
                """,
                TENANT_ID,
