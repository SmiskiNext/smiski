...
class TenantInstalledEventConsumerIntegrationTest {

    private static final String TOPIC = "tenant.tenant.installed";
    private static final String TYPE = "io.github.smiskinext.tenant.v1.installed";
    ...
    void awaitConsumerAssignment() {
        await().atMost(AWAIT_TIMEOUT)
                .until(() -> endpointRegistry.getListenerContainers().stream()
                        .allMatch(TenantInstalledEventConsumerIntegrationTest::hasAssignment));
    }
    ...
    @Test
    void consumedInstalledEventUpsertsActiveRow() {
        String cloudId = "cloud-it-" + UUID.randomUUID();
        String data = """
                {"cloudId":"%s","installationId":"inst-1","appId":"app-1",
                "updatedAt":"%s","status":"ACTIVE"}""".formatted(cloudId, Instant.now());
        ...
        await().atMost(AWAIT_TIMEOUT).untilAsserted(() -> {
            ...
            assertThat(entity.get().getStatus()).isEqualTo(TenantStatus.ACTIVE.name());
        });
    }

    @Test
    void redeliveredInstalledEventIsIdempotent() {
        String cloudId = "cloud-it-idem-" + UUID.randomUUID();
        String data = """
                {"cloudId":"%s","installationId":"inst-1","updatedAt":"%s","status":"ACTIVE"}""".formatted(cloudId, Instant.now());
