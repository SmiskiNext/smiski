...
import io.github.smiskinext.meet.infrastructure.messaging.TenantKafkaProperties;
...
/**
 * Kafka configuration for the tenant lifecycle consumers.
 *
 * <p>Both consumers (installed and uninstalled) share one {@link ConsumerFactory} and one
 * {@link ConcurrentKafkaListenerContainerFactory}. Failed messages are retried a bounded number of
 * times before being routed to a per-topic dead-letter topic, so a persistent failure never blocks
 * the partition.
 *
 * <p>The DLT recoverer reuses the existing {@code cloudEventKafkaTemplate} bean from
 * {@link KafkaProducerConfig} to avoid a duplicate {@code KafkaTemplate<String, CloudEvent>} bean.
 */
@Configuration
@EnableKafka
...
public class TenantKafkaConfig {
    ...
    public DefaultErrorHandler tenantConsumerErrorHandler(
            @Qualifier("cloudEventKafkaTemplate") KafkaTemplate<String, CloudEvent> kafkaTemplate,
