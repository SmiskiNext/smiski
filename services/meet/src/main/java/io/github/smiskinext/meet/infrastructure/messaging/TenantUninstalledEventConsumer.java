 ...
 * {@code TenantUninstalled} proto rendered as proto-JSON. This consumer parses the {@code data}
 * back into the typed {@link TenantUninstalled} proto message via {@link JsonFormat} and delegates
 * to {@link HandleTenantUninstalledUseCase}. Messages that cannot be decoded are thrown so the
...
public class TenantUninstalledEventConsumer {
    ...
    @KafkaListener(
            ...
            groupId = "${app.tenant.kafka.uninstalled-consumer-group:meet-tenant-uninstalled}",
            containerFactory = "tenantKafkaListenerContainerFactory")
    public void onMessage(CloudEvent event) {
        log.debug("Received tenant.tenant.uninstalled event id={}", event.getId());
        TenantUninstalled proto = decode(event);
