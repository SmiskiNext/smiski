...
/**
 * Accepts a meeting invitation on behalf of its owning invitee.
 *
 * <p>Loads the meeting and the target invitee, authorizes the acting account as the invitee owner,
 * applies the domain transition, and publishes the enriched accepted event through the outbox.
 */
@Service
public class AcceptMeetingInviteeApplicationService implements AcceptMeetingInviteeUseCase {

    private final MeetingRepository meetingRepository;
    private final MeetingInviteeRepository meetingInviteeRepository;
    private final EventPublisher eventPublisher;

    public AcceptMeetingInviteeApplicationService(
    ...
    @Override
    @Transactional
    public Result<AcceptMeetingInviteeResult, MeetingError> execute(
            AcceptMeetingInviteeCommand command) {
        ...
        Result<Void, MeetingError> transition =
                invitee.accept(InviteeResponseSupport.context(meeting));
        ...
        return Result.success(new AcceptMeetingInviteeResult(
                invitee.getId().value(),
