...
/**
 * Registry of SSE emitters with a single shared daemon heartbeat scheduler, holding two
 * independent registries that share the heartbeat and timeout configuration:
 *
 * <ul>
 ...
 *
 * <p>Implements both {@link SseRelayPort} (push side) and {@link SseSubscriptionPort} (subscribe
 * side). Emitter completion, timeout, and error all remove the emitter and cancel its heartbeat
 * task so no resources leak. Thread-safe via {@link ConcurrentHashMap} +
 * {@link CopyOnWriteArrayList}.
 */
@Component
public class SseConnectionManager implements SseRelayPort, SseSubscriptionPort {
    ...
    @Override
    public void pushJoinRequestCreated(UUID meetingId, PendingJoinRequest request) {
