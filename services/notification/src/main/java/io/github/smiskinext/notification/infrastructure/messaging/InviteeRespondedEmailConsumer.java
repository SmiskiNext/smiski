...
public class InviteeRespondedEmailConsumer {
    ...
    public void onMessage(CloudEvent event) {
        ReplyCalendar calendar;
        try {
            calendar = decode(event);
    ...
    private ReplyCalendar decodeAccepted(CloudEventData data) {
    ...
    private ReplyCalendar decodeDeclined(CloudEventData data) {
    ...
    private ReplyCalendar decodeTentative(CloudEventData data) {
    ...
    private static String replySubject(ReplyCalendar calendar) {
        String title = calendar.title() == null ? "Meeting" : calendar.title();
        return calendar.inviteeName() + " responded to " + title;
    }

    private static String replyBody(ReplyCalendar calendar) {
        return calendar.inviteeName() + " (" + calendar.inviteeEmail() + ") responded "
                + calendar.responseStatus() + ". The attached calendar reply updates their status.";
    }
