...
/**
 * Read-model projection of a tenant synchronised from the tenant service via Kafka.
 *
 ...
 * @param siteUrl the Atlassian site URL used to construct Jira deep-links, or {@code null}
 * @param status current lifecycle status
 * @param updatedAt timestamp of the last projection update
 * @param uninstalledAt timestamp when the tenant was uninstalled, or {@code null} if active
 * @param purgeAfter deadline after which tenant data may be purged, or {@code null} if not set
 */
public record TenantProjection(
        String tenantId,
        String cloudId,
